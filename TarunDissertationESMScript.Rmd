---
title: "Tarun's Dissertation ESM Script"
author: "Tarun Vermani MSc"
date: "2022-08-16"
...
MFQ_Cons <- ggplot(Mood_table, aes(x = cons_mean, y = mfq)) + 
  xlab("Average Consummatory Pleasure (1-7)") +
  ylab("MFQ Score") +
 scale_x_continuous(breaks = seq(0, 7, by = 1))+
...
MFQ_Ant <- ggplot(Mood_table, aes(x = anticipation_mean, y = mfq)) + 
  xlab("Average Anticipatory Pleasure (1-7)") +
  ylab("MFQ Score") +
 scale_x_continuous(breaks = seq(0, 7, by = 1))+
  scale_y_continuous(breaks = seq(0, 60, by = 10))+
  geom_point() +
...
#Regression Ant vs MFQ

summary(lm(mfq ~ anticipation_mean, Mood_table))
#Regression Cons vs MFQ

summary(lm(mfq ~ cons_mean, Mood_table))
```
...
```{r}
#Creating data frame for correlation matrix 

Positive_correlation <- final.beep %>%
  dplyr::select(Enthusiastic, Relaxed, Cheerful, Satisfied) 

Negative_correlation <- final.beep %>%
  dplyr::select(Ashamed, Annoyed, Anxious, Lonely, Insecure, Down, Guilty)
#Pos Correlations
cor(Positive_correlation, use = "complete.obs")
#Neg Correlations
cor(Negative_correlation, use = "complete.obs")
```
